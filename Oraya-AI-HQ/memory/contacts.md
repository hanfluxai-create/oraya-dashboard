# Contacts

## Oraya Labs

- **Claude (AI)** — `orayaassistant@gmail.com`. Sends briefings to both founders.
  - Recipients: `chitrakshmayank3@gmail.com` (Chitraksh), `manifestdestiny247@gmail.com` (Frankie).

- **Frankie Alicea** — CEO. See `frankie-profile.md`.

- **Chitraksh Mayank** — CTO. See `chitraksh-profile.md`.

## Pilot (Mercedes-Benz of South Bay / AutoNation)

- **Chris Mazzio** — Sales Manager. `Mazzioc@autonation.com`, 310.650.4826.
  - 3311 Pacific Coast Highway, Torrance, CA 90505.
  - Official dealership email path.
  - **Status Apr 22:** entering week 3 of warm silence since card capture Apr 18. Walk-in protocol armed if no reply to text + voicemail by 3 PT today. Script at `deliverables/demo-playbook/mazzio-in-person-script.md`. Product is demo-stable (500+ call-tested appt-setter + Teresa demo Apr 20 ran cleanly); Mazzio is no longer blocked on product. If walk-in triggers, final outreach cadence ends Apr 28 before recasting dormant.

- **Khalid** — Frankie's entry point at Mercedes. 10% / $700K offer structure.
  - **DO NOT** route via official Mercedes email. Separate crypto/shell-company arrangement.
  - Status + deadlines live in dashboard Tasks tab.

## Investors

- **Jared** — $5K into Oraya for engineering. Convertible SAFE drafted, likely unsigned/unnotarized. **Legal risk.**
- **Abdo Melek** — $5K, same terms as Jared. Must locate executed doc.
- **Uzi Sassoon** — Former HNW investor. 7 exits, Samsung sale. Multi-vertical insurance RPA interest. Reactivable for Quote AI.
- **Braxton** — Health insurance contact via Uzi's network.

## Vendors / Partners

- **Registered agent** — Oraya Corp renewal filing. Status + deadline in dashboard.
- **Insurance carriers** — Frankie signing at 110% commission. 140% at $60-100K/mo issued premium.
- **Lead buyers** — P2 $40/lead resale program, not yet established.
- **India CA (Chartered Accountant)** — Lives in Chitraksh's building. Free consult on US/India tax optimization + entity structuring.

## Chitraksh personal network

- Parents (India), sister (UK), brother, friend with US bank (payment workarounds).

## Agency network (Frankie)

10-20 personal relationships with agency owners running 50-100 person teams. First sale channel for P2 lead gen.

## P2 anchor producer

- **Teresa** — insurance producer, earns $40-50K/mo. Extended network = 3 agency owners.
  - **Status Apr 22:** Demo ran cleanly Apr 20 PM on the Mercedes appt-setter. Anchor-close pending today (T+2). Three-branch single question: (a) 2 warm leads in 7 days under referral code `T-APR26-R1`, (b) 1 agency-owner intro this week with 10% year-one ARR referral fee, (c) $1,000/week retainer starting May 1 (4-week prepay via Stripe, 7-day cancel). Fallback "let me think" moves her from anchor-producer to lukewarm-deferred, no outreach before Apr 28. Playbook at `deliverables/demo-playbook/teresa-close-terms.md`.
